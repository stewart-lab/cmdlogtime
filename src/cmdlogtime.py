...
def get_args_from_line(line, sep="|"):
    ...
    if len(line_copy) == 0:
        ...
        sys.exit(1)
    if len(vals) != NUM_VALS_IN_LINE:
        print(
            "Problem in command file for line: \n",
            line,
            f"\nThere must be {NUM_VALS_IN_LINE} values separated by {NUM_VALS_IN_LINE-1} '",
            sep,
            f"' symbols. You need {NUM_VALS_IN_LINE-1} delimiters, even if some values are null.",
        )
        sys.exit(1)
...
def massage_and_validate_args(args, start_time_secs, pretty_start_time, command_line_def_file):
    ...
    the_out_dir = ""
    with open(command_line_def_file, "r") as cmd_line_f:
        skip_headers(cmd_line_f)  # first_line at this point will contain the description line.
        for line in cmd_line_f:
            if line:
                arg_defs = get_args_from_line(line)
                ...
                new_args[tmp_name] = args.__dict__[
                    tmp_name
                ]  # if it is a directory or file,  new_args[tmp_name] will be overlain below
                if arg_defs["is_dir"] == "1" or arg_defs["is_file"] == "1":
                    if args.__dict__[tmp_name]:
                        # rms. I don't like this.  I think I need a different way to check that the flagged arg is
                        # NOT filled in, versus filled in incorrectly
                ...
                if arg_defs["check_file"] == "1":  # same goes for files, see rms comment 2 lines above.
                    if not args.__dict__[tmp_name].endswith(
                        "ZZZ"
    ...
    assert the_out_dir != ""
    if "rerun_out_directory" in new_args and new_args["rerun_out_directory"] is not None:
    ...
    addl_logfile, parms_logfile, script_logfile, pkgs_logfile = build_log_files(
...
def get_args(start_time_secs, pretty_start_time, command_line_def_file):
    ...
    my_args = massage_and_validate_args(
        parser.parse_args(), start_time_secs, pretty_start_time, command_line_def_file
    )
    return my_args
...
def get_time_and_pretty_time():
    ...
    time_secs = time.mktime(
        time_struct
    )  # need to use something besides time.mktime if want fractions of sec.
